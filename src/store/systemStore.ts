  ...
  interface SystemState {
    ...
    isSpeaking: boolean;               // <--- ADD THIS BACK
  ...
  export const useSystemStore = create<SystemState>((set, get) => ({
    ...
    mode: 'active',
    isProcessing: false,
    isSpeaking: false,                 // <--- ADD THIS BACK
    setIsSpeaking: (val) => set({ isSpeaking: val }), // <--- ADD THIS BACK
    threatLevel: 'safe',
    vaultSkills: [
    ...
    toggleMode: async () => {
      ...
      try {
        // 1. Tell Python to swap models
        await invoke('set_monitoring_mode', { mode: newMode });
        set({ mode: newMode });
        
        // 2. NATIVE TAURI WINDOW CONTROL
        const bubbleWindow = WebviewWindow.getByLabel('bubble');
        if (bubbleWindow) {
          if (newMode === 'passive') {
            await bubbleWindow.show();
            await bubbleWindow.setAlwaysOnTop(true); // Forces it over other desktop apps
          } else {
            await bubbleWindow.hide();
          }
        } else {
          console.warn("Could not find window labeled 'bubble'. Check tauri.conf.json.");
        }
      } catch (error: any) {

...
interface BubbleWSPayload {
  type: 'tts' | 'threat' | 'telemetry' | 'code_diff';
  is_speaking?: boolean;
  volume?: number;
  level?: string;
  source?: string;
...
export default function PassiveBubble() {
  const {
    isProcessing, mode, threatLevel, systemVRAM, permissions,
    togglePermission, user, logout, backendConnected, setBackendConnected
  } = useSystemStore();
  ...
  // ======================================================================
  // 1. CONNECTION HEALTH CHECK (Port 8000)
  // ======================================================================
  ...
  // ======================================================================
  // 2. SOVEREIGN WEBSOCKET (Leak‑proof)
  // ======================================================================
  useEffect(() => {
    let reconnectTimer: ReturnType<typeof setTimeout>;

    const connectWS = () => {
      wsRef.current = new WebSocket('ws://127.0.0.1:8000/ws/bubble');
      ...
      wsRef.current.onerror = (err) => console.warn("Bubble WS Error:", err);
    };

    connectWS();

    return () => {
      // Prevent ghost reconnect after unmount
      if (wsRef.current) {
  ...
  // ======================================================================
  // 3. TAURI EVENT LISTENERS (Promise‑array pattern)
  // ======================================================================
  useEffect(() => {
    const unlistenPromises = [
      listen('threat_detected', (event: any) => {
      ...
      listen('code_diff_available', (event: any) => {
        const payload = event.payload as { data: CodeDiffPayload };
        setActiveDiff(payload.data);
        setShowDiffModal(true);
        if (payload.data.threat_level === 'critical' || payload.data.threat_level === 'high') {
          setIsThundering(true);
          setParticleCount(16);
          setTimeout(() => { setIsThundering(false); setParticleCount(8); }, 3000);
        }
  ...
  // ======================================================================
  // 4. CLOSE CONTEXT MENU ON OUTSIDE CLICK
  // ======================================================================
  useEffect(() => {
  ...
  // ======================================================================
  // 5. WINDOW DRAG FUNCTIONALITY (only on bubble itself)
  // ======================================================================
  ...
  // ======================================================================
  // 6. APPLY CODE FIX (with path sanitisation)
  // ======================================================================
  ...
  const visualState = useMemo(() => {
    ...
    if (!backendConnected || connectionStatus === 'disconnected') {
      ...
      coreColor = 'bg-gradient-to-br from-[#ff003c] to-[#990000] shadow-[0_0_50px_rgba(255,0,60,1)] animate-pulse';
      ringColor = 'border-[#ff003c] opacity-100 scale-125 border-dashed';
  ...
  // ======================================================================
  // 10. PARTICLE RENDERER (optimised)
  // ======================================================================
  const renderParticles = useMemo(() => {
    const maxParticles = Math.min(particleCount, 12);
  ...
  return (
    <div
      className="fixed inset-0 w-full h-full flex items-center justify-center z-50 overflow-hidden pointer-events-none"
      role="application"
    ...
    >
      ...
      {/* Context Menu (pointer‑events‑auto so it can be clicked) */}
      {showContextMenu && (
        <div
          ref={contextMenuRef}
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 panel-3d rounded-xl border border-[#1f1f1f] bg-[#0a0a0a]/95 backdrop-blur-xl z-50 overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200 pointer-events-auto"
          role="menu"
        ...
        >
          ...
          <div className="p-2 space-y-1 max-h-80 overflow-y-auto gloomy-scroll">
            <button
              ...
              role="menuitem"
            >
              <div className="flex items-center gap-3"><Eye size={14} /><span>Vision OCR</span></div>
            ...
            <button
              ...
              role="menuitem"
            >
              <div className="flex items-center gap-3"><Mic size={14} /><span>Voice Control</span></div>
            ...
            <button
              ...
              role="menuitem"
            >
              <div className="flex items-center gap-3"><Code size={14} /><span>IDE Integration</span></div>
            ...
            <button
              ...
              role="menuitem"
            >
              <div className="flex items-center gap-3"><Globe size={14} /><span>Browser Proxy</span></div>
            ...
            <button
              ...
              role="menuitem"
            >
              <Zap size={14} /><span>Scan Screen Now</span>
            </button>
            ...
            <button
              ...
              role="menuitem"
            >
              <Maximize2 size={14} /><span>Open Dashboard</span>
            </button>
            ...
            {user && (
              <>
                ...
                <button
                  ...
                  role="menuitem"
                >
                  <LogOut size={14} /><span>Sever Connection</span>
                </button>
              </>
            )}
          </div>
      ...
      {/* Main Bubble – draggable and clickable */}
      <div
        ref={bubbleRef}
        ...
        onKeyDown={(e) => {
          if (e.key === 'Enter' || e.key === ' ') handleBubbleClick(e as any);
        }}
      >
        {/* Particles */}
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center">{visualState.icon}</div>
        </div>
